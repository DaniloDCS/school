...
const { IDgenerator } = _functions2.default.call(void 0, );
 ...
 async function init() {
  await _connection2.default.execute(`CREATE TABLE IF NOT EXISTS UserRole (
  ...
  ['Stuudy', 'Danilo', 'Pelurio', 'Lorena', 'Diogo', 'Estudante', 'Professor', 'Diretor', 'Coordenador'].forEach( async ( title, index ) => {
    await _connection2.default.execute( `INSERT INTO Users (id, name, birthday, email, password, username, role) SELECT "${IDgenerator( 10 )}", "${title}", "2001-08-06", "${title.toLowerCase()}@stuudy.com", "${await _bcryptjs2.default.hash( "stuudy", 10 )}", "@${title.toLowerCase()}", ${index < 5 ? 0 : (index - 4)} WHERE NOT EXISTS (SELECT * FROM Users WHERE username = "@${title.toLowerCase()}")` );
  });
  ...
  // TODO: FAZER
  await _connection2.default.execute(`CREATE TABLE IF NOT EXISTS UsersFriends (
    id TEXT PRIMARY KEY NOT NULL,
